...
class FixedOrderSCM(nn.Module):
    ...
    def noise_to_causal_det(self,epsilon,adjacency_matrix):
        return self._solve(epsilon,adjacency_matrix=adjacency_matrix)
...
class MLPVariableOrderCausalModel(VariableOrderSCM):
    ...
    def log_prob_noise_weakly_supervised(self, epsilon1, epsilon2, intervention, adjacency_matrix,include_intervened=True,
        ...
        if include_intervened:
            ...
            adjacency_matrix=adjacency_matrix.repeat(epsilon2.shape[0],1,1)
            z_intervened, log_det = self.noise_to_causal_det(epsilon2,adjacency_matrix=adjacency_matrix)
